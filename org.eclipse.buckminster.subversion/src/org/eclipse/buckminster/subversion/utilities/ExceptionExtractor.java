...
/**
 * This class is used to autogenerate the "svn_exception_messages.properties"
 * file. This file is located in org.eclipse.buckminster.subversion package and
 * used by SvnExceptionHandler class to analyze the exceptions thrown by
...
public class ExceptionExtractor {
	...
	private static final String COMMENT = "This file is autogenerated from " + ExceptionExtractor.class.getName() //$NON-NLS-1$
	...
	static {
		try {
			MESSAGES.load(ExceptionExtractor.class.getResourceAsStream("exception_messages.properties")); //$NON-NLS-1$
		} catch (IOException e) {
			e.printStackTrace();
	...
	/**
	 * Each argument points to root of subversion sources folder. Version is
	 * extracted from the forlder's name ( expecting "subversion-version" )
	 * 
	 * @param args
	 * @throws IOException
	 */
	public static void main(String[] args) throws IOException {
		...
		for (String folder : args) {
			...
			String version = folderName.split("-")[1]; //$NON-NLS-1$
			versions.add(version);
			// building the default language
			new ExceptionExtractor(version);
			// building variants
			final File[] files = gatherPoFiles(new File(folder + "/subversion/po")); //$NON-NLS-1$
			for (File file : files)
		...
		builder.append('\n');
		TRANSLATIONS.store(new FileOutputStream("src/org/eclipse/buckminster/subversion/svn_exception_messages.properties"), builder.toString()); //$NON-NLS-1$
	}
	...
	private static File[] gatherPoFiles(File file) {
		...
		return file.listFiles(new FilenameFilter() {
			...
			public boolean accept(File dir, String name) {
				return name.endsWith(".po"); //$NON-NLS-1$
			}
	...
	private static String getLangage(File file) {
		...
		final int indexOf = name.indexOf(".po"); //$NON-NLS-1$
	...
	protected ExceptionExtractor(String version, File file) throws IOException {
		...
		final InputStreamReader reader = new InputStreamReader(new FileInputStream(file), "UTF-8"); //$NON-NLS-1$
		System.out.println(">> reading " + file.getAbsolutePath()); //$NON-NLS-1$
		try {
	...
	private String getLongestTextPart(String translation) {
		final String[] split = translation.split("%ld|%s"); //$NON-NLS-1$
		int maxLenght = 0;
		int maxIndex = 0;
	...
	private String getTranslation(BufferedReader reader) throws IOException {
		...
		while ((line = reader.readLine()).indexOf("msgstr") == -1) //$NON-NLS-1$
			;
