...
public class ComponentInfoProvider implements IComponentInfoProvider
{
	...
	public String prepareHTML(Map<String, String> properties, OPML opml, String destination) throws Exception
	{
		...
		InputStream is = getResource(IPath.SEPARATOR + HTML_FOLDER + IPath.SEPARATOR + HTML_TEMPLATE);
		...
		if(!new File(imgDestDir, IMG_ARROW).exists())
			FileUtils.copyFile(getResource(IPath.SEPARATOR + IMG_FOLDER + IPath.SEPARATOR + IMG_ARROW), imgDestDir,
					IMG_ARROW, m_nullMonitor);
		if(!new File(imgDestDir, IMG_FOOTER).exists())
			FileUtils.copyFile(getResource(IPath.SEPARATOR + IMG_FOLDER + IPath.SEPARATOR + IMG_FOOTER), imgDestDir,
					IMG_FOOTER, m_nullMonitor);
		if(!new File(imgDestDir, IMG_EMPTY_PROFILE).exists())
			FileUtils.copyFile(getResource(IPath.SEPARATOR + IMG_FOLDER + IPath.SEPARATOR + IMG_EMPTY_PROFILE),
					imgDestDir, IMG_EMPTY_PROFILE, m_nullMonitor);
		if(!new File(imgDestDir, IMG_FAVICON).exists())
			FileUtils.copyFile(getResource(IPath.SEPARATOR + IMG_FOLDER + IPath.SEPARATOR + IMG_FAVICON), imgDestDir,
