...
/**
 * Entry point for running Ant builds inside Eclipse (within the same JRE). Clients may instantiate this class; it is
 * not intended to be subclassed.
 * <p/>
 * <div class="TableSubHeadingColor"> <b>Usage note:</b><br/>
 * As opposed to {@link org.eclipse.ant.core.AntRunner} this class will not set up a new classloader for each call and
 * consequently, it does not support the customClassPath.
 * <p>
 * Refer to the "Platform Ant Support" chapter of the Programmer's Guide section in the Platform Plug-in Developer Guide
...
public class AntRunner
{
	 ...
	 * reference in the Ant Project (<code>org.apache.tools.ant.Project.getReferences()</code>). A long- running task
	 * could, for example, get the monitor during its execution and check for cancellation. The key value to retrieve
	 * the progress monitor instance is <code>AntCorePlugin.ECLIPSE_PROGRESS_MONITOR</code>.
	 * 
	...
	/**
	 * Sets the build logger. The parameter <code>className</code> is the class name of an
	 * <code>org.apache.tools.ant.BuildLogger</code> implementation. The class will be instantiated at runtime and the
	 * logger will be called on build events (<code>org.apache.tools.ant.BuildEvent</code>). Only one build logger is
	 * permitted for any build.
	 * 
	 * <p>
