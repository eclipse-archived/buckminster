...
public interface IVersionConverter extends IBuckminsterExtension {
	...
	/**
	 * Converts the <code>version</code> into a <code>BranchOrTag</code> that
	 * can be understood by a source code control system. An implementation will
	 * typically use the <code>version</code> as a tag or branch name, possibly
	 * modified using a substitution pattern
	 * 
	 * @param version
	...
	/**
	 * Converts the <code>branchOrTag</code> into a <code>IVersion</code>. This
	 * is the reverse of {@link #createSelector(IVersion) }.
	 * 
	 * @param branchOrTag
