...
{
	private static final Pattern s_labelExp = Pattern.compile("^([a-zA-Z_][a-zA-Z0-9_.-]*)$");
	...
	throws Exception
	{
		...
		m = s_timestampExp.matcher("2005-07-24");
		if(m.matches())
		{
			int c = m.groupCount();
			for(int i = 1; i <= c; ++i)
		...
		m = s_timestampExp.matcher("20050724T101523");
		if(m.matches())
		{
			int c = m.groupCount();
			for(int i = 1; i <= c; ++i)
