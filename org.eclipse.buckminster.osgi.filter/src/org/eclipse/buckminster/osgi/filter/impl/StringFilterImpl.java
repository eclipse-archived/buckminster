...
class StringFilterImpl extends FilterImpl
{
	...
	private static class SetAccessibleAction implements PrivilegedAction<Object>
	{
		...
		public Object run()
		{
			m_accessible.setAccessible(true);
			return null;
		}
	}
	...
	void toString(StringBuilder sb)
	{
		...
		case APPROX:
			sb.append("~="); //$NON-NLS-1$
			sb.append(encodeValue(approxString(m_value.toString())));
			break;
		}
		sb.append(')');
	}
	...
	{
		boolean boolval2 = Boolean.parseBoolean(m_value.trim());
	...
	private boolean compare(byte byteval)
	{
		...
		try
		{
			byteval2 = Byte.parseByte(m_value.trim());
		}
	...
	private boolean compare(double doubleval)
	{
		...
		try
		{
			doubleval2 = Double.parseDouble(m_value.trim());
		}
		catch(NumberFormatException e)
		{
			return false;
	...
	private boolean compare(float floatval)
	{
		...
		try
		{
			floatval2 = Float.parseFloat(m_value.trim());
		}
		catch(NumberFormatException e)
		{
			return false;
	...
	private boolean compare(int intval)
	{
		...
		try
		{
			intval2 = Integer.parseInt(m_value.trim());
		}
		catch(NumberFormatException e)
		{
			return false;
	...
	private boolean compare(long longval)
	{
		...
		try
		{
			longval2 = Long.parseLong(m_value.trim());
		}
		catch(NumberFormatException e)
		{
			return false;
	...
	private boolean compare(short shortval)
	{
		...
		try
		{
			shortval2 = Short.parseShort(m_value.trim());
		}
		catch(NumberFormatException e)
		{
			return false;
	...
	private boolean compare(String string)
	{
		...
		{
		case EQUAL:
			return string.equals(m_value);
		case APPROX:
			string = approxString(string);
			String string2 = approxString(m_value);

			return string.equalsIgnoreCase(string2);
		case GREATER:
			return string.compareTo(m_value) >= 0;
		default:
			return string.compareTo(m_value) <= 0;
		}
	}
	...
	private boolean compareAsComparable(Comparable<Object> value1)
	{
		try
		{
			...
			Object value2 = constructor.newInstance(new Object[] { m_value.trim() });
