...
import org.polarion.team.svn.client.javasvn.JavaSVNClientProxy;
...
public class SubversiveSession {
	private ISVNClientWrapper m_clientWrapper;
	 ...
	 */
	public SubversiveSession(String repositoryURI, String branch, String tag) throws CoreException {
		try {
			...
			m_clientWrapper = new JavaSVNClientProxy();
	...
	public long getLastChangeNumber() throws CoreException {
		try {
			DirEntry[] entries = m_clientWrapper.list(this.getSVNUrl(null).toString(), Revision.HEAD, Revision.HEAD, false, false, new SVNNullProgressMonitor());
			return entries[0].lastChangedRevision;
	...
	public ISVNClientWrapper getSVNClientWrapper() {
	...
	SVNURL getSVNRootUrl(boolean branches) throws MalformedURLException {
		...
		try {
			return SVNURL.parseURIEncoded(bld.toString());
		} catch (SVNException e) {
			throw new MalformedURLException("Illegal URL: " + bld.toString());
		}
	}
	...
	public boolean isBranchesSession() {
		return m_branch != null;
	}

	public boolean isTagsSession() {
		return m_tag != null;
	}

	/**
	 * Return the URL lead-in for the URL of this session. The URL lead-in is
	 * the part of the URL before the trunk/tags/branches. <p/>
	 * 
	 * @return Example:when the URL is "http://eclipse.org/buckminster/trunk/"
	 *         then this method will return "http://eclipse.org/buckminster"
	 */
	public String getURLLeadIn() {
		return m_urlLeadIn;
	}
}
