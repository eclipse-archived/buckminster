...
public class ImportOperation extends WorkspaceJob
{
	 ...
	 * <p>
	 * The <code>provider</code> parameter allows this operation to deal with the source object in an abstract way. This
	 * operation calls methods on the provider and the provider in turn calls specific methods on the source object.
	 * </p>
	 * <p>
	 * The <code>filesToImport</code> parameter specifies what file system objects are to be imported.
	 * </p>
	 * <p>
	 * The default import behavior is to recreate the complete container structure for the file system objects in their
	 * destination. If <code>setCreateContainerStructure</code> is set to <code>false</code>, then no container
	 * structure is created for each of the file system objects.
	 * </p>
	 * 
	 * @param containerPath
	 ...
	 * <p>
	 * The <code>source</code> parameter represents the root file system object to import. All contents of this object
	 * are imported. Valid types for this parameter are determined by the supplied <code>IImportStructureProvider</code>
	 * .
	 * </p>
	 * <p>
	 * The <code>provider</code> parameter allows this operation to deal with the source object in an abstract way. This
	 ...
	 * </p>
	 * <p>
	 * The default import behavior is to recreate the complete container structure for the contents of the root file
	 * system object in their destination. If <code>setCreateContainerStructure</code> is set to false then the
	 * container structure created is relative to the root file system object.
	 * </p>
	 * 
	 * @param containerPath
	 *            the full path of the destination container within the workspace
	...
	/**
	 * Creates a new operation that imports specific file system objects. In this usage context, the specified source
	 * file system object is used by the operation solely to determine the destination container structure of the file
	 * system objects being imported.
	 * <p>
	 * The <code>source</code> parameter represents the root file system object to import. Valid types for this
	 * parameter are determined by the supplied <code>IImportStructureProvider</code>. The contents of the source which
	 * are to be imported are specified in the <code>filesToImport</code> parameter.
	 * </p>
	 * <p>
	 * The <code>provider</code> parameter allows this operation to deal with the source object in an abstract way. This
	 * operation calls methods on the provider and the provider in turn calls specific methods on the source object.
	 * </p>
	 * <p>
	 * The <code>filesToImport</code> parameter specifies what contents of the root file system object are to be
	 * imported.
	 * </p>
	 * <p>
	 * The default import behavior is to recreate the complete container structure for the file system objects in their
	 * destination. If <code>setCreateContainerStructure</code> is set to <code>false</code>, then the container
	 * structure created for each of the file system objects is relative to the supplied root file system object.
	 * </p>
	 * 
	 * @param containerPath
	 *            the full path of the destination container within the workspace
	...
	/**
	 * Returns the status of the import operation. If there were any errors, the result is a status object containing
	...
	/*
	 * (non-Javadoc) Method declared on WorkbenchModifyOperation. Imports the specified file system objects from the
	 * file system.
	 */
	@Override
	public IStatus runInWorkspace(IProgressMonitor progressMonitor)
	{
		...
		return (errorTable.size() == 0)
				? Status.OK_STATUS
				: getStatus();
	}

	/**
	 * Sets whether the containment structures that are implied from the full paths of file system objects being
	 * imported should be duplicated in the workbench.
	 * 
	 * @param value
	 *            <code>true</code> if containers should be created, and <code>false</code> otherwise
	 */
	...
	/**
	 * Sets whether imported file system objects should automatically overwrite existing workbench resources when a
	 * conflict occurs.
	 * 
	 * @param value
	 *            <code>true</code> to automatically overwrite, and <code>false</code> otherwise
	 */
	...
	/**
	 * Prompts if existing resources should be overwritten. Recursively collects existing read-only files to overwrite
	 * and resources that should not be overwritten.
	 * 
	 * @param sourceStart
	 ...
	 * @param noOverwrite
	 *            files that were selected to be skipped (don't overwrite). object type IPath
	 * @param overwriteReadonly
	 *            the collected existing read-only files to overwrite. object type IPath
	 * @param policy
	...
	void collectExistingReadonlyFiles(IPath sourceStart, List sources, ArrayList noOverwrite,
			ArrayList overwriteReadonly, int policy)
	{
	...
	/**
	 * Creates the folders that appear in the specified resource path. These folders are created relative to the
	...
	/**
	 * Deletes the given resource. If the resource fails to be deleted, adds a status object to the list to be returned
	 * by <code>getResult</code>.
	 * 
	 * @param resource
	 *            the resource
	 */
	...
	void importFile(Object fileObject, int policy)
	{
		...
		try
		{
			 ...
			 * provider).getResourceAttributes(fileObject)); } catch (CoreException e) { errorTable.add(e.getStatus());
			 * } }
			 */
		}
		catch(CoreException e)
	...
	/**
	 * Imports the specified file system objects into the workspace. If the import fails, adds a status object to the
	 * list to be returned by <code>getStatus</code>.
	 * 
	 * @param filesToImport
	 *            the list of file system objects to import (element type: <code>Object</code>)
	...
	/**
	 * Queries the user whether the resource with the specified path should be overwritten by a file system object that
	 * is being imported.
	 * 
	 * @param resourcePath
	 *            the workspace path of the resource that needs to be overwritten
	 ...
	 * 
	 * @param shell
	 *            context for use by the VCM provider to prompt user for check-out. The user will not be prompted if set
	 *            to <code>null</code>.
	...
	/**
	 * Validates the given file system objects. The user is prompted to overwrite existing files. Existing read-only
	 * files are validated with the VCM provider.
	 * 
	 * @param sourceFiles
	 *            files to validate
	 */
	...
	/**
	 * Creates the folders that appear in the specified resource path assuming that the destinationContainer begins at
	 * the root. Do not create projects.
	 * 
	 * @param path
	 *            the relative path of the resource
	 * @return the container resource coresponding to the given path
