...
public class TPWizard extends AdvancedWizard
{
	...
	public boolean performFinish()
	{
		...
		catch(Exception e)
		{
			...
			HelpLinkErrorDialog.openError(null, m_installWizard.getWindowImage(),
					MaterializationConstants.ERROR_WINDOW_TITLE, "Materialization failed",
					MaterializationConstants.ERROR_HELP_TITLE, m_installWizard.getErrorURL(),
	...
	protected void addAdvancedPages()
	{
		...
		/*
		 * temporarily removed - links to updatesites are used instead m_toolSelectionPage = new TPToolSelectionPage();
			...
			getContainer().run(true, false, new IRunnableWithProgress()
			{
				public void run(IProgressMonitor monitor) throws InvocationTargetException, InterruptedException
				{
					...
					try
					{
						...
						m_currentEclipseVersion = VersionFactory.createVersion(IVersionType.OSGI,
								featureModel.getFeature().getVersion()).replaceQualifier(null);
						...
						m_spacesInstalled = PDECore.getDefault().getFeatureModelManager().findFeatureModel(
								SPACES_FEATURE) != null;
						m_rssowlInstalled = PDECore.getDefault().getFeatureModelManager().findFeatureModel(
								RSSOWL_FEATURE) != null;
					}
					...
					{
						throw new InvocationTargetException(new JNLPException(
								"Error while analysing Eclipse installation", null));
					}
					finally
					{
						unsetTP();
	...
	// TODO make it work with distros instead of update sites
	/*
	 * TPWizardPage getToolsSelectionPage() { return m_toolSelectionPage; }
	 */
	...
	private MaterializationSpecBuilder getMspec(String mspecURL) throws JNLPException
	{
		...
		try
		{
			...
			{
				throw new JNLPException("Cannot create URL link to a temp file",
						ERROR_CODE_MALFORMED_PROPERTY_EXCEPTION, e);
			}
		}
		catch(Exception e)
