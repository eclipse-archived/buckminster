...
/**
 * The abstract implementation of a selection dialog. It can be primed with initial selections (
 * <code>setInitialSelections</code>), and returns the final selection (via <code>getResult</code>) after completion.
 * <p>
...
public abstract class SelectionDialog extends TrayDialog
{
	...
	/**
	 * Creates a dialog instance. Note that the dialog will have no visual representation (no widgets) until it is told
	 * to open.
	 * 
	 * @param parentShell
	 *            the parent shell
	 ...
	 * 
	 * @return the array of selected elements, or <code>null</code> if Cancel was pressed
	 */
	...
	/**
	 * Set the dialog settings that should be used to save the bounds of this dialog. This method is provided so that
	 * clients that directly use SelectionDialogs without subclassing them may specify how the bounds of the dialog are
	 * to be saved.
	 * 
	 * @param settings
	 *            the {@link IDialogSettings} that should be used to store the bounds of the dialog
	 * 
	 * @param strategy
	 *            the integer constant specifying how the bounds are saved. Specified using
	 *            {@link Dialog#DIALOG_PERSISTLOCATION} and {@link Dialog#DIALOG_PERSISTSIZE}.
	 * 
	 * @since 3.2
	 * 
	...
	/**
	 * Gets the dialog settings that should be used for remembering the bounds of the dialog, according to the dialog
	 * bounds strategy. Overridden to provide the dialog settings that were set using
	 * {@link #setDialogBoundsSettings(IDialogSettings, int)}.
	 * 
	 * @return the dialog settings used to store the dialog's location and/or size, or <code>null</code> if the dialog's
	 *         bounds should not be stored.
	 * 
	 * @since 3.2
	 * 
	 * @see Dialog#getDialogBoundsStrategy()
	...
	/**
	 * Get the integer constant that describes the strategy for persisting the dialog bounds. Overridden to provide the
	 * dialog bounds strategy that was set using {@link #setDialogBoundsSettings(IDialogSettings, int)}.
	 * 
	 * @return the constant describing the strategy for persisting the dialog bounds.
	 * 
	 * @since 3.2
	 ...
	 * 
	 * @param newResult
	 *            list of selected elements, or <code>null</code> if Cancel was pressed
	 */
