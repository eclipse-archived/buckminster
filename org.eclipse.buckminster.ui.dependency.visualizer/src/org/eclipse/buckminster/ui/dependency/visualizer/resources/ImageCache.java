...
/**
 * The ImageCache works as plugin internal image cache for the resources
 * consumed by the visualization.
 * <p>
 * All resources stay cached until an explizit call to dispose.
 * <p>
 * Note that all methods are static so calling dispose will affect all consumers
 * in the same context.
 * 
...
public class ImageCache {
	...
	/**
	 * Tries to load an image from the given path and caches the result in an
	 * internal map.
	 * <p>
	 * If the path was requested before, the image is fetched from the cache
	 * instead of newly allocated. The resources stay in memory until
	 * {@link ImageCache#dispose()} is called.
	 * <p>
	 * If the image at the given path cannot be found, <code>null</code> is
	 * returned.
	 * 
	 * @param path
