...
public class OpenDepencencyGraphAction extends AbstractCSpecAction {
	...
	protected void run(CSpec cspec, Shell shell) {
		...
		final ComponentRequest request = new ComponentRequest(cspec.getComponentIdentifier().getName(), cspec.getComponentIdentifier()
					...
					activePart.getSite().getShell().getDisplay().asyncExec(new Runnable() {
						...
						public void run() {
							try {
								...
								UiUtils.openError(activePart.getSite().getShell(), Messages.OpenDepencencyGraphAction_OpenEditorErrorMessage, e);
							}
						}
					});
