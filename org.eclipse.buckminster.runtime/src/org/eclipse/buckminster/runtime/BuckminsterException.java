 ...
 * used exceptions. The idea is to normalize exception into a
 * {@link CoreException}
 * 
...
public abstract class BuckminsterException extends CoreException {
	...
	public static IStatus createStatus(Throwable cause) {
		return (cause instanceof CoreException) ? ((CoreException) cause).getStatus() : new Status(IStatus.ERROR, Buckminster.PLUGIN_ID, IStatus.OK,
	...
	public static Throwable unwind(Throwable t) {
		for (;;) {
			...
			//
			if (tc != RuntimeException.class && tc != InvocationTargetException.class && tc != SAXException.class && tc != IOException.class)
				break;
